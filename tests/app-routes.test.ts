...
describe("Route Definitions", () => {
  ...
  it("should export all required route type definitions", () => {
    ...
    expect(appRoutes.some((r) => r.path === RoutePath.ForgotPassword)).toBe(
      true
    );
    expect(appRoutes.some((r) => r.path === RoutePath.ResetPassword)).toBe(
      true
    );
  });
});
...
describe("Authentication Routes", () => {
  ...
  it("should parse auth action query parameters for various modes", () => {
    const verifyEmailRoute = getRoute(
      "/auth-action?mode=verifyEmail&oobCode=abc123&continueUrl=http://example.com"
    );
    ...
    const resetPasswordRoute = getRoute(
      new URL(
        "http://example.com/auth-action?mode=resetPassword&oobCode=xyz789"
      )
    );
  ...
  it("should parse verify email query parameters", () => {
    const verifyEmailRoute = getRoute(
      "/verify-email?registered=true&code=abc123"
    );
  ...
  it("should parse forgot password query parameters", () => {
    const forgotPasswordRoute = getRoute(
