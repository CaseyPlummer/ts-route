...
describe("pageTitle", () => {
  ...
  it("should generate correct titles for routes with query parameters and fragments", () => {
    expect(pageTitle("/register?param=value#fragment")).toBe(
      `Register • ${appName}`
    );
    expect(pageTitle("/@user?search=term#section")).toBe(
      `Dashboard • ${appName}`
    );
    expect(pageTitle("/auth-action?mode=verifyEmail&oobCode=abc123")).toBe(
      `Authentication Action • ${appName}`
    );
    expect(pageTitle("/forgot-password?email=user@example.com")).toBe(
      `Forgot Password • ${appName}`
    );
  });

  it("should handle URL objects for title generation", () => {
    expect(pageTitle(new URL("http://example.com/"))).toBe(`Home • ${appName}`);
    expect(pageTitle(new URL("http://example.com/register"))).toBe(
      `Register • ${appName}`
    );
  });
...
describe("getRoute", () => {
  ...
  it("should throw errors for invalid or non-existent routes", () => {
    expect(() => getRoute("/non-existent-path")).toThrow(
      "Route not found for path: non-existent-path"
    );
    expect(() => getRoute("http://")).toThrow(
      "A valid page URL is required to get the route"
    );
    expect(() => getRoute("https://")).toThrow(
      "A valid page URL is required to get the route"
    );
    // @ts-ignore Testing
...
describe("getHref", () => {
  ...
  it("should generate correct hrefs for dynamic routes with parameters", () => {
    expect(
      getHref(RoutePath.Dashboard, { params: { handle: "user123" } })
    ).toBe("/@user123");
  });

  it("should encode special characters in parameters and query parameters", () => {
    expect(
      getHref(RoutePath.Dashboard, { params: { handle: "user name" } })
    ).toBe("/@user%20name");
  ...
  it("should include query parameters and fragments in hrefs", () => {
    ...
    expect(getHref(RoutePath.Home, { fragment: "section1" })).toBe(
      "/#section1"
    );
    expect(
      getHref(RoutePath.VerifyEmail, {
  ...
  it("should handle edge cases for query parameters and empty inputs", () => {
    ...
    expect(
      getHref(RoutePath.VerifyEmail, { query: { registered: true } })
    ).toBe("/verify-email?registered=true");
    expect(
      getHref(RoutePath.VerifyEmail, { query: { registered: false } })
    ).toBe("/verify-email?registered=false");
...
describe("getRouteForPaths", () => {
  ...
  function toUrl(href: string): URL {
    return new URL(
      href.startsWith("http") ? href : `https://example.com${href}`
    );
  }
  ...
  it("should throw when the path is not in the allowed set", () => {
    ...
    expect(() =>
      getRouteForPaths(
        new URL(
          authHref.startsWith("http")
            ? authHref
            : `https://example.com${authHref}`
        ),
        tuple
      )
...
describe("app-specific route helpers (core buildHref integration)", () => {
  it("core buildHref preserves casing (fragment & percent-encodings unchanged)", () => {
    ...
    expect(href).toBe(
      "/core/React%20%26%20Typescript?filter=Category%3AWeb#Results"
    );
  });
