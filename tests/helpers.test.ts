...
describe("normalizePercentEscapes", () => {
  ...
  it("should handle mixed valid and invalid escapes", () => {
    expect(normalizePercentEscapes("valid%20and%invalid")).toBe(
      "valid%20and%25invalid"
    );
  });
...
describe("encodeReservedChars", () => {
  ...
  it("should encode semicolons, colons, and at signs", () => {
    expect(encodeReservedChars("user@host:port;param")).toBe(
      "user%40host%3Aport%3Bparam"
    );
  });
...
describe("processQueryValues", () => {
  ...
  it("should remove duplicates preserving first occurrence order", () => {
    expect(processQueryValues(["a", "b", "a", "c", "b"])).toEqual([
      "a",
      "b",
      "c",
    ]);
  ...
  it("should preserve order of unique values", () => {
    expect(processQueryValues(["zebra", "apple", "banana"])).toEqual([
      "zebra",
      "apple",
      "banana",
    ]);
  ...
  it("should handle values with internal spaces", () => {
    expect(processQueryValues(["  hello world  ", "hello world"])).toEqual([
      "hello world",
    ]);
  });
...
function makeRoute(
  partial: Partial<Route<"test", DummyQP, Q>>
): Route<"test", DummyQP, Q> {
  return {
    path: "test",
    title: () => "Test",
    ...partial,
  ...
  it("should support complex assembly (ordering, arrays, selective omission)", () => {
    const route = makeRoute({
      serializeQuery: (q: any) => {
        ...
        if (q.arr?.length) {
          parts.push(
            q.arr.map((v: any) => `arr=${encodeURIComponent(v)}`).join("&")
          );
        }
        if (q.a && q.b) {
    ...
    const route = makeRoute({
      serializeQuery: (_q: any, { queryParams }: any) => {
        ...
        if (arrVals && arrVals.length > 1) {
          parts.push(
            arrVals.map((v: any) => `arr=${encodeURIComponent(v)}`).join("&")
          );
        }
...
describe("buildPath", () => {
  ...
  it("should handle params with special characters", () => {
    ...
    expect(buildPath(route, { query: "hello world" })).toBe(
...
describe("findRoute", () => {
  it("should parse query and params", () => {
    ...
    const r = findRoute(
      "/@h/collections/abc?sort=date&filter=a&filter=b",
      routes as Route<string>[]
    );
  ...
  it("should use custom queryParamsFactory when provided", () => {
    const getValue = (key: string, raw: Record<string, string[]>) =>
      raw[key]?.[0];
    const getValues = (key: string, raw: Record<string, string[]>) =>
      raw[key] || [];
...
describe("getRoute", () => {
  ...
  it("should throw error with invalid URL type", () => {
    ...
    expect(() => getRoute(null, routes)).toThrow(
  ...
  it("should throw error showing available routes when not found", () => {
    ...
    expect(() => getRoute("/missing", routes)).toThrow(
      "Route not found for path: missing"
    );
  });
...
describe("buildHref", () => {
  it("should build href with params + query + fragment (core only)", () => {
    ...
    expect(href).toBe(
      "/search/react%20%26%20typescript?filter=category%3Aweb#Results"
    );
  });
...
describe("getHref", () => {
  it("should build href from path string", () => {
    const href = getHref(
      "alpha",
      routes,
      {} as unknown as { params?: object; query?: object; context?: object }
    );
  ...
  it("should throw error when route path not found", () => {
    ...
    expect(() => getHref("missing", routes, {})).toThrow(
      "Route not found for path: missing"
    );
  });
...
describe("buildNestedRoutes", () => {
  ...
  it("should handle orphaned children (parent not found)", () => {
    const routes = [
      { path: "child", parentPath: "missing", title: () => "Child" },
    ] as Route<string>[];
    const nested = buildNestedRoutes(routes);
...
describe("buildBreadcrumbTrail", () => {
  ...
  it("should pass context only to matched route", () => {
    const routes = [
      {
        path: "parent",
        title: ({ context }: { context?: { value?: string } }) =>
          `Parent: ${context?.value ?? "none"}`,
      },
      {
        path: "child",
        parentPath: "parent",
        title: ({ context }: { context?: { value?: string } }) =>
          `Child: ${context?.value ?? "none"}`,
      },
    ] as Route<string>[];
