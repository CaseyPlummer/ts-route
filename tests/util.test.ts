...
describe("getRelativePart", () => {
  ...
  it("should extract pathname without leading slash from string URL", () => {
    ...
    expect(getRelativePart("https://example.com/some/nested/path")).toBe(
      "some/nested/path"
    );
  });
  ...
  it("should extract pathname from URL object", () => {
    ...
    expect(
      getRelativePart(new URL("https://example.com/some/nested/path"))
    ).toBe("some/nested/path");
  });

  it("should include search parameters", () => {
    expect(getRelativePart("https://example.com/path?key=value")).toBe(
      "path?key=value"
    );
    expect(
      getRelativePart("https://example.com/path?key=value&other=123")
    ).toBe("path?key=value&other=123");
  });

  it("should include hash fragments", () => {
    expect(getRelativePart("https://example.com/path#section")).toBe(
      "path#section"
    );
    expect(getRelativePart("https://example.com/path#section-id")).toBe(
      "path#section-id"
    );
  });

  it("should include both search parameters and hash fragments", () => {
    expect(getRelativePart("https://example.com/path?key=value#section")).toBe(
  ...
  it("should handle URLs with encoded characters", () => {
    expect(getRelativePart("https://example.com/path%20with%20spaces")).toBe(
      "path%20with%20spaces"
    );
    expect(getRelativePart("https://example.com/user?name=John%20Doe")).toBe(
      "user?name=John%20Doe"
    );
  });

  it("should handle URLs with special characters", () => {
    expect(getRelativePart("https://example.com/path-with-hyphens")).toBe(
      "path-with-hyphens"
    );
    expect(getRelativePart("https://example.com/path_with_underscores")).toBe(
      "path_with_underscores"
    );
    expect(getRelativePart("https://example.com/path.with.dots")).toBe(
      "path.with.dots"
    );
  });
...
describe("pathToRegex", () => {
  ...
  it("should throw on null/undefined", () => {
    ...
    expect(() => pathToRegex(undefined)).toThrow(
      "pathToRegex: path is null or undefined"
    );
    // @ts-ignore deliberate invalid
    expect(() => pathToRegex(null)).toThrow(
      "pathToRegex: path is null or undefined"
    );
  });
  ...
  it("should throw on non-string", () => {
    // @ts-ignore deliberate invalid
    expect(() => pathToRegex(123)).toThrow(
      "pathToRegex: path must be a string"
    );
  });
...
describe("extractParamNames", () => {
  it("should extract names from path with params", () => {
    expect(extractParamNames("users/[id]/orders/[orderId]")).toEqual([
      "id",
      "orderId",
    ]);
  });
...
describe("getQueryValues", () => {
  it("should return distinct trimmed values", () => {
    ...
    expect(
      getQueryValues(params, "key").sort((a: string, b: string) =>
        a.localeCompare(b)
      )
    ).toEqual(["a", "b"]);
  });
